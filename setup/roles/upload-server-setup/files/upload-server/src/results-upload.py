...
V3_QUIC_DATA_FILE = '{date}-v3_quic_results.jsonl'
HTTP3_DATA_FILE = '{date}-http3_results.jsonl'
DNSURSERDATA_FILE = '{date}-dns-user-info.jsonl'
DNSDATA_FILE = '{date}-dns-results.jsonl'
OUTPUT_DIR = '/var/he-upload-server/results-data'
V2OUTPUT_DIR = '/var/he-upload-server/v2-results-data'
V3_QUIC_OUTPUT_DIR = '/var/he-upload-server/v3-quic-results'
HTTP3_OUTPUT_DIR = '/var/he-upload-server/http3-availability-results'
...
@app.route('/results/v3-quic', methods=['POST'])
def upload_v3_quic_data():
    ...
    try:
        ...
        date_str = date.strftime('%Y-%m-%d')
        month_dir = date.strftime('%Y/%m')
        data_filepath = os.path.join(V3_QUIC_OUTPUT_DIR, month_dir, V3_QUIC_DATA_FILE.format(date=date_str))

        os.makedirs(os.path.join(V3_QUIC_OUTPUT_DIR, month_dir), exist_ok=True)
...
def upload_http3_availability_data():
    # Get JSON data from request
    try:
        data = request.get_json()
        ...
        date_str = date.strftime('%Y-%m-%d')
        month_dir = date.strftime('%Y/%m')
        data_filepath = os.path.join(HTTP3_OUTPUT_DIR, month_dir, HTTP3_DATA_FILE.format(date=date_str))

        os.makedirs(os.path.join(HTTP3_OUTPUT_DIR, month_dir), exist_ok=True)
...
@app.route('/results/dnsresults', methods=['POST'])
def upload_dnsdata():
    # Get JSON data from request
    try:
        data = request.get_json()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--v3-quic-output-directory', required=True, type=str, help='Directory where v3-quic results are stored')
    parser.add_argument('--http3-output-directory', required=True, type=str, help='Directory where HTTP/3 availability results are stored')
    args = parser.parse_args()
    OUTPUT_DIR = args.output_directory
    V2OUTPUT_DIR = args.v2_output_directory
    DNSOUTPUT_DIR = args.dns_output_directory
    V3_QUIC_OUTPUT_DIR = args.v3_quic_output_directory
    HTTP3_OUTPUT_DIR = args.http3_output_directory
